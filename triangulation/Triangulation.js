...
canvas.addEventListener("click",function(event){
    ...
    var Coordinates={"x":event.x-290,y:event.y};
	var x,y;
	x=Coordinates.x;
	y=Coordinates.y-80;
    ...
    if (typeof Polygon_Coordinates[max - 1] !== "undefined") { 
        ...
        Canvas_Context.moveTo(Previous_Coordinate.x, Previous_Coordinate.y-80);
        Canvas_Context.lineTo(Current_Coordinate.x, Current_Coordinate.y-80);
...
var delay_time=2000;//Variable to change 
var d_t=1000;       //the speed animation
...
function Change_Color(x1,y1,x2,y2,x3,y3){
	...
	Draw_Point(x1,y1-80,"lawngreen");
	Draw_Point(x2,y2-80,"#2D4C89");
	Draw_Point(x3,y3-80,"#2D4C89");
	},d_t)
}
...
function Update_Color(x1,y1,x2,y2,x3,y3,count){
	window.setTimeout(function(){
	Draw_Point(x1,y1-80,"yellow");
	Draw_Point(x2,y2-80,"yellow");
	Draw_Point(x3,y3-80,"yellow");
	draw_line(count);
...
$("button").on("click",function(){
	...
	Canvas_Context.beginPath();
	Canvas_Context.moveTo(Polygon_Coordinates[Polygon_Coordinates.length-1].x,Polygon_Coordinates[Polygon_Coordinates.length-1].y-80);
	Canvas_Context.lineTo(Polygon_Coordinates[0].x,Polygon_Coordinates[0].y-80);
	Canvas_Context.lineWidth=3.0;
    Canvas_Context.stroke();
    start();
})
